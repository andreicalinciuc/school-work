class MyString
{
  private:
    ...
    void strncpy (char *dest, const char *src, unsigned int length);
  ...
  public:
    ...
    MyString* SubString (unsigned int start, unsigned int size); // returneaza un obiect nou MyString in care copie din obiectul curent sub-sirul care incepe de la pozitia start si are lunginea size. daca pozitia start e in afara sirului meu, sau dimensiunea ceruta (size) e prea mare, functia returneaza NULL
   bool Delete (unsigned int start, unsigned int size); // sterge caracterele din sir de la pozitia start, de dimensiunea size. Returneaza true daca parametri sunt corecti. daca pozitia start e in afara sirului meu, sau dimensiunea ceruta (size) e prea mare, functia returneaza false 
    ...
    bool Insert (unsigned int index, const char *text); // insereaza textul text la pozitia index in sir. Daca index e in afara index-ului returneaza false; face si redimensionare daca e cazul
